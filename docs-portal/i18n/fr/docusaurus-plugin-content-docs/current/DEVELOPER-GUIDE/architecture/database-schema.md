...
description: "Conseils étape par étape pour le schéma de base de données dans PrimeCalendar."
...
> **Résumé rapide** : cette page explique le schéma de base de données dans PrimeCalendar à l'aide d'étapes pratiques et de conseils de dépannage.
...
Utilisez ce guide pour compléter le schéma de base de données de manière fiable. Confirmez les résultats attendus après chaque étape avant de passer aux paramètres avancés facultatifs.
...
**Scénario** : votre équipe a besoin d'un comportement cohérent pour le schéma de base de données.
...
- `02-ARCHITECTURE/database-schema.md` : Cette page a été déplacée vers la structure consolidée. - Page canonique : DEVELOPER-GUIDE/architecture/database-schema.md - Instantané archivé : archives/pre-consolidation/02-ARCHITECTURE/database-schema.md
