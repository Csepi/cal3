...
export class UserPermissionsController {
  ...
  async getUserPermissions(@Req() req) {
    const permissions = await this.userPermissionsService.getUserPermissions(req.user.id);
    ...
    return {
      ...
      editableReservationCalendarIds: permissions.editableReservationCalendarIds,
      viewableReservationCalendarIds: permissions.viewableReservationCalendarIds,
  ...
  @ApiResponse({ status: 200, description: 'Accessible organizations retrieved successfully' })
  ...
  @ApiOperation({ summary: 'Get reservation calendars accessible to current user' })
  @ApiResponse({ status: 200, description: 'Accessible reservation calendars retrieved successfully' })
  async getAccessibleReservationCalendars(@Req() req) {
    return await this.userPermissionsService.getUserAccessibleReservationCalendars(req.user.id);
  }
}