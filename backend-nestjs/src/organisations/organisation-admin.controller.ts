...
export class OrganisationAdminController {
  constructor(private readonly organisationAdminService: OrganisationAdminService) {}
  ...
  async assignOrganisationAdmin(
    ...
    const orgAdmin = await this.organisationAdminService.assignOrganisationAdmin(
      organisationId,
  ...
  async removeOrganisationAdmin(
    ...
    await this.organisationAdminService.removeOrganisationAdmin(organisationId, userId, user);
  ...
  async getOrganisationAdmins(@Param('id', ParseIntPipe) organisationId: number) {
    const admins = await this.organisationAdminService.getOrganisationAdmins(organisationId);
  ...
  async removeUserFromOrganisation(
    ...
    await this.organisationAdminService.removeUserFromOrganisation(organisationId, userId, user);
  ...
  @Get(':id/users')
  @UseGuards(OrganisationAdminGuard)
  async getOrganisationUsers(@Param('id', ParseIntPipe) organisationId: number) {
    const users = await this.organisationAdminService.getOrganisationUsers(organisationId);
  ...
  async getUserOrganisationAdminRoles(@GetUser() user: User) {
    const adminRoles = await this.organisationAdminService.getUserOrganisationAdminRoles(user.id);
  ...
  async checkOrganisationAdminStatus(
    ...
    const isAdmin = await this.organisationAdminService.isOrganisationAdmin(user.id, organisationId);
