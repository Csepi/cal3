...
async function extractSchemaFromEntities() {
  ...
  for (const entity of entities) {
    ...
    for (const column of entity.columns) {
      ...
      const typeStr = column.length ? `${column.type}(${column.length})` : String(column.type);
    ...
    if (entity.relations.length > 0) {
      ...
      for (const relation of entity.relations) {
        ...
        const joinColumns = relation.joinColumns.map(jc => jc.databaseName).join(', ');
        const inverseSide = relation.inverseSidePropertyPath || 'N/A';

        const relationInfo: any = {
          propertyName: relation.propertyName,
          type: relationType,
          targetEntity,
          joinColumns,
          inverseSide,
          cascadeOptions: relation.isCascadeInsert || relation.isCascadeUpdate || relation.isCascadeRemove,
        };

        tableInfo.relations.push(relationInfo);

        console.log(`    ${relation.propertyName.padEnd(30)} ${relationType.padEnd(20)} → ${targetEntity}`);
        if (joinColumns) {
  ...
  console.log('\n📋 TABLE LIST:');
  for (const table of schemaReport.tables) {
    console.log(`  - ${table.tableName.padEnd(40)} (${table.columns.length} columns, ${table.relations.length} relations)`);
  }
